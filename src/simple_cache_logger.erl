...
-export([configure/0, set_primary_level/1, set_handler_output/1, set_output_format/1,
         set_output_format/2]).
...
%% @doc Configures Logger according to the application environment.
-spec configure() -> ok.
configure() ->
...
set_handler_output(HandlerId) ->
    ...
    ok =
        logger:add_handler(HandlerId,
                           logger_std_h,
                           #{config => #{file => filename:join(?LOGS_DIRECTORY, ?LOG_FILE)}}).

%% @doc Sets the output format for the default Logger handler.
-spec set_output_format(output_format()) -> ok.
set_output_format(Format) ->
    set_output_format(?DEFAULT_HANDLER_ID, Format).

%% @doc Sets the output format for the specified Logger handler.
-spec set_output_format(logger:handler_id(), output_format()) -> ok.
set_output_format(HandlerId, modern) ->
    logger:update_formatter_config(HandlerId, #{single_line => true, legacy_header => false});
set_output_format(HandlerId, legacy) ->
    logger:update_formatter_config(HandlerId, #{single_line => false, legacy_header => true}).
...
ensure_log_dir() ->
    filelib:ensure_dir(
        filename:join(?LOGS_DIRECTORY, ?LOG_FILE)),
    ok.
